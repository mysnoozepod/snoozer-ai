...
async function planTrustedAdvisorTurnWithModel({ requestId, query = "", context = {} } = {}) {
  ...
  const systemContent = [
    ...
    "Use compound_fact_answer when the shopper requests more than one protected fact or a policy-only fact, product_sizes for an exact size question, durability_objection for lifespan/wear questions, rewards_explanation for natural rewards, points, badge, milestone, progress, or offer questions, store_value for why-buy-from-us questions, and recommendation_explanation for why an assessment or pod was chosen.",
    ...
    "For follow-ups about them, the other one, the first one, or those mattresses, preserve activeJourney.comparisonProductHandles unless the shopper explicitly changes a subject.",
    "Reject only the product the shopper explicitly rejects. A request for a softer or more supportive alternative changes direction but does not reject a newly suggested product.",
    "Questions about what to notice while testing mattresses are sleep_education or product_experience guidance, not medical or tiredness framing unless the shopper raised those concerns.",
    "For recap language such as where we landed, what we decided, or what to test next, use confusion_recovery with recap_current_state and suggest_one_next_step.",
    "Valid requestedFacts include recommendation_reasons, product_sizes, product_features, warranty, delivery, returns, financing, rewards, price, availability, compatibility, cart, durability, and store_value.",

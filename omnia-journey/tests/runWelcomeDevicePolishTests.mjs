...
assert.match(results, /data-results-lead-reason="true"/);
assert.match(results, /buildPodReasonText\(\{/);
assert.match(results, /data-results-secondary-reason="true"/);
assert.match(results, /data-results-snoozer="true"/);
assert.match(results, /src="\/snoozer-avatar\.png"/);
assert.match(results, /import welcomeBrandMarkSrc from "@\/assets\/mysnoozepod-logo-welcome\.png"/);
assert.match(results, /imageSrc=\{welcomeBrandMarkSrc\}/);
assert.match(results, /loading="eager"/);
assert.match(results, /useReducedMotion/);
assert.match(results, /data-results-status=\{state\}/);
assert.doesNotMatch(results, /Your strongest match based on your sleep profile\./);
assert.doesNotMatch(results, /#1A66D2/i);

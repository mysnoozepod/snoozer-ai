...
test("Pod Customize follows the four core-only step variants", async ({ page }) => {
  ...
  const choose = async (label, position = "first") => {
  ...
  await expect(page.locator('[data-sleep-essentials-step], [data-sleep-essentials-card]')).toHaveCount(0);
  await expect(page.getByText(/Continue to Essentials|Back to essentials/i)).toHaveCount(0);
});

test("old saved Essentials state migrates to core Review without an accessory request", async ({ page }) => {
  await page.setViewportSize({ width: 1180, height: 820 });
  let accessoryCatalogRequests = 0;
  await page.route("**/sleepEssentials/catalog", async (route) => {
    accessoryCatalogRequests += 1;
    await route.abort();
  });
  await page.addInitScript(() => {
    ...
    sessionStorage.setItem("snooze.podBuilder.4", JSON.stringify({
      size: "Full",
      baseType: "platform",
      motionType: "standard",
      dcLeft: "",
      dcRight: "",
      stepKey: "essentials",
      confirmed: { size: true, base: true, motion: true },
      selectedEssentials: { pillows: { handle: "old-pillow", variantId: "gid://shopify/ProductVariant/999" } },
      skippedEssentials: { sheets: true },
      essentialsVersion: 2,
      assessmentSignature,
    }));
  });

  await page.goto("/pod/pod-4?podLayoutState=build-essentials", { waitUntil: "domcontentloaded" });
  await ensureRendered(page);
  await expect(page.locator('[data-pod-builder-state="review"]')).toBeVisible();
  await expect(page.locator('[data-pod-builder-summary-row="mattress"]')).toContainText("Full");
  await expect(page.locator('[data-pod-builder-summary-row="base-motion"]')).toContainText("Platform Base");
  await expect(page.locator('[data-sleep-essentials-step], [data-sleep-essentials-card], [data-pod-builder-summary-row="essential"]')).toHaveCount(0);
  await expect(page.getByText(/Back to essentials|Sleep Essentials/i)).toHaveCount(0);
  expect(accessoryCatalogRequests).toBe(0);

  const migrated = await page.evaluate(() => JSON.parse(sessionStorage.getItem("snooze.podBuilder.4")));
  expect(migrated.stepKey).toBe("review");
  expect(migrated.size).toBe("Full");
  expect(migrated.baseType).toBe("platform");
  expect(migrated.selectedEssentials).toBeUndefined();
  expect(migrated.skippedEssentials).toBeUndefined();
  expect(migrated.essentialsVersion).toBeUndefined();
});

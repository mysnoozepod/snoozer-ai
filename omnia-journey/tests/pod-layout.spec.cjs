...
async function writeMeasurementArtifacts(page, viewport, state, measurement) {
  ...
  const viewportScreenshotPath = path.join(dir, `${state}-viewport.png`);
  const headerScreenshotPath = path.join(dir, `${state}-header.png`);
  const navScreenshotPath = path.join(dir, `${state}-navigation.png`);
  const heroScreenshotPath = path.join(dir, `${state}-hero.png`);
...
async function generateSummaryReport() {
  ...
  rows.forEach(({ viewport, state, measurement }) => {
    if (!measurement) {
      lines.push(`| ${viewport} | ${state} | missing | missing | missing | missing | missing | missing | missing | missing |`);
      return;
    }
    ...
    lines.push(
      `| ${viewport} | ${state} | ${pageScroll} | ${activeTop} | ${activeVisible} | ${heroChildren} | ${shellScrollClip} | ${overlap} | ${primary} | ${warnings} | ${result} |`
    );
  });
...
for (const viewport of VIEWPORTS) {
  for (const state of STATES) {
    test(`${viewport.name} ${state}`, async ({ page }, testInfo) => {
      ...
      await page.goto(`/pod/pod-4?podLayoutState=${state}`, { waitUntil: "domcontentloaded" });
